...
function Step1AWSSetup({ data, update, next }) {
  ...
  // Start deployment and get external_id on mount
  useEffect(() => {
    const start = async () => {
      setLoading(true);
      setError('');
      try {
        const { deployment_id, external_id } = await externalIdService.startDeployment();
        update({ deployment_id, external_id });
      } catch (err) {
  ...
  const validateARN = async () => {
    ...
    setError('');
    try {
      const result = await externalIdService.validateARN(data.deployment_id, data.aws_arn);
      if (!result.valid) {
        setError(result.error || 'Invalid ARN or external ID. Please check your credentials.');
        return;
      }
      // Update external_id in case backend returns a new one
